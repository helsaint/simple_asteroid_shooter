...
class LaserMeteorCollisionHandler(CollisionHandler):
    def __init__(self, meteors, lasers, collision_sound,
                 world_width: int, world_height: int, grid_cell_size: int):
        super().__init__(meteors, lasers, collision_sound,
    ...
    def dispatch_events(self, event, x, y):
        if event:
            dispatcher.dispatch_event(Events.METEOR_DESTROYED, 
                                      {"explosion_sound": self.collision_sound,
                                       "increment score": 1,
                                       "pos": (x, y),
                                       })
